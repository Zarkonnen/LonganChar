...
public class Lenet4hNet {
	...
	public Lenet4hNet() {
		...
		for (int i = 0; i < Lenet4g.OUTPUT_SIZE; i++) {
			h5.nodes.add(new Node("Pre-output " + i));
		}
		...
		// Connect h4 to pre-output (full connection)
		int h5i = 0;
		for (Node h5N : h5.nodes) {
			int h4i = 0;
			for (Node h4N : h4.nodes) {
				if (h4i / 4 != h5i % 3 && h4i / 4 != h5i % 5) { continue; }
				Weight w = new Weight(rnd(-2.0 / 10, 2.0 / 10));
				h4.weights.add(w);
