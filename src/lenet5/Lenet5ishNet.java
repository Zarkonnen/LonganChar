...
public class Lenet5ishNet {
	...
	public Lenet5ishNet() {
		...
		Layer h5 = new Layer("H5 (preprocessing)");
		for (int i = 0; i < LenetX2.OUTPUT_SIZE; i++) {
			h5.nodes.add(new Node("Preprocessing " + i));
		}
		...
		// Connect h4 to h5 (slightly symmetry-broken connection)
		int h4i = 0;
		for (Node h4N : h4.nodes) {
			...
			for (Node h5N : h5.nodes) {
				h5i++;
				if (h4i % 2 != h5i % 3 && h4i % 16 != h5i / 5) {
					Weight w = new Weight(rnd(-2.0 / 120, 2.0 / 120));
					h4.weights.add(w);
		...
		for (Node h5N : h5.nodes) {
			for (Node oN : output.nodes) {
				Weight w = new Weight(rnd(-2.0 / 129, 2.0 / 129));
				h5.weights.add(w);
