...
public class Lenet4fNet {
	...
	public Lenet4fNet() {
		...
		for (int i = 0; i < 3; i++) {
			input.nodes.add(new Node("additional input " + i));
		}
		// Bias node!
		...
		for (Node h4N : h4.nodes) {
			for (Node oN : output.nodes) {
				Weight w = new Weight(rnd(-2.0 / 196, 2.0 / 196));
				h4.weights.add(w);
		...
		// Add additionals to output.
		for (int i = 28 * 28; i < 28 * 28 + 3; i++) {
			for (Node oN : output.nodes) {
				Weight w = new Weight(rnd(-2.0 / 196, 2.0 / 196));
				h4.weights.add(w);
				new Connection(input.nodes.get(i), oN, w);
			}
		}
